...
export const addStopsdb = async (req, res) => {
  ...
  try {
    ...
    const newSequence =
      routeStops && routeStops.length > 0 ? routeStops[0].sequence + 1 : 1;
...
//   console.log("Adding route:", { routeName, routeNumber });
    ...
    try {
      ...
      const { data: routeData, error: routeError } = await supabase
        ...
        .select("id") // Assuming 'route_id' is the correct column name
        .single();
      ...
      // Step 2: Insert associations into 'route_yatayat' table (without vehicle_timing)
      const yatayatEntries = [
        { route_id: routeId, yatayat_id: 1 },
        { route_id: routeId, yatayat_id: 2 },
        { route_id: routeId, yatayat_id: 3 },
      ];
      ...
      res.json({
        success: true,
        message: "Route and associated yatayat added successfully",
      });
    } catch (error) {
      res.status(500).json({ success: false, message: error.message });
    }
